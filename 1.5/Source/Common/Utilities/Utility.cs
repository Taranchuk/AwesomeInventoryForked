                ...
                for (int i = 0; i < allParts.Count; i++)
                {
                    ...
                    if (allParts[i].depth == BodyPartDepth.Outside &&
                                            (allParts[i].coverage >= 0.1 ||
                                             ...
                                             allParts[i].def == AwesomeInventoryBodyPartDefOf.Neck))
                    {
        ...
        public static float CalculateArmorByPartsCE(Pawn pawn, StatDef stat, ref string text, string unit)
        {
            ...
            if (num > 0.005f)
            {
                ...
                for (int i = 0; i < bpList.Count; i++)
                {
                    ...
                    if (part.depth == BodyPartDepth.Outside && (part.coverage >= 0.1 || (part.def == BodyPartDefOf.Eye || part.def == AwesomeInventoryBodyPartDefOf.Neck)))
                    {
                        text += part.LabelCap + ": ";
