...
namespace AwesomeInventory.UI
{
    ...
    public abstract class DrawGearTabWorker : IDrawGearTab
    {
        ...
        protected virtual void DrawLoadoutButtons(Pawn selPawn, float x, ref float rollingY, float width)
        {
            ...
            {
                List<FloatMenuOption> loadoutOptions = BuildMenuOptions(LoadoutManager.Loadouts.Where(l => l.GetType() == typeof(AwesomeInventoryLoadout)).OfType<ApparelPolicy>().ToList());
                List<FloatMenuOption> outfitOptions = BuildMenuOptions(Current.Game.outfitDatabase.AllOutfits.Where(o => o.GetType() != typeof(AwesomeInventoryCostume)).ToList() ?? Enumerable.Empty<ApparelPolicy>().ToList());
                ...
                {
                    if (selPawn.outfits?.CurrentApparelPolicy is AwesomeInventoryLoadout)
                    {
                        Find.WindowStack.Add(
                            AwesomeInventoryServiceProvider.MakeInstanceOf<Dialog_ManageLoadouts>(selPawn.outfits.CurrentApparelPolicy, selPawn, false));
                    }
                    else
                    {
                        openLoadout = true;
                ...
                if (selPawn.outfits?.CurrentApparelPolicy is AwesomeInventoryLoadout loadout)
                {
                    ...
                    if (row.ButtonText(costumeButtonText))
                    {
                        ...
                        if (costumes.Any())
                        {
                            Find.WindowStack.Add(
                                new FloatMenu(
                                    BuildMenuOptions(costumes.OfType<ApparelPolicy>().ToList())
                                    .Concat(new FloatMenuOption(
                                        UIText.DontUseCostume.TranslateSimple()
                                        , () =>
                                        {
                                            selPawn.outfits.CurrentApparelPolicy = (loadout as AwesomeInventoryCostume)?.Base ?? loadout;
                ...
                string selectButtonText = selPawn.outfits?.CurrentApparelPolicy != null
                                          ? (selPawn.outfits.CurrentApparelPolicy.GetType() != typeof(AwesomeInventoryCostume))
                                            ? selPawn.outfits.CurrentApparelPolicy.label
                                            : (selPawn.outfits.CurrentApparelPolicy as AwesomeInventoryCostume).Base.label
            ...
            List<FloatMenuOption> BuildMenuOptions(IList<ApparelPolicy> outfits)
            {
                ...
                for (int i = 0; i < outfits.Count; i++)
                {
                    ...
                    options.Add(new FloatMenuOption(
                        ...
                        () =>
                        {
                            selPawn.outfits.CurrentApparelPolicy = outfits[local_i];
